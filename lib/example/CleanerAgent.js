...
class CleanerAgent extends Agent {
    ...
    send() {
        ...
        if (newViewKey == "DOWN") {
            ...
            if (this.posBrain0[1] + this.posBrain[1] + 1 > this.generatedWorld.length - 1) generarAbajo(this.generatedWorld);
            //suma 1 en la posicion donde estaba
            this.generatedWorld[this.posBrain0[1] + this.posBrain[1] - 1][this.posBrain0[0] + this.posBrain[0]]++;
        }
        ...
        if (newViewKey == "RIGHT") {
            ...
            if (this.posBrain0[0] + this.posBrain[0] + 1 > this.generatedWorld[0].length - 1) generarDerecha(this.generatedWorld);
            //suma 1 en la posicion donde estaba
            this.generatedWorld[this.posBrain0[1] + this.posBrain[1]][this.posBrain0[0] + this.posBrain[0] - 1]++;
        }
...
function direccion(viewKey = [], mundoRecorrido = [], pos0 = [], pos = []) {
    ...
    if (izquierda == "w" || matIzq == "w") matIzq = null;

    if (arriba == "w" || matArr == "w") matArr = null;

    if (derecha == "w" || matDer == "w") matDer = null;

    if (abajo == "w" || matAba == "w") matAba = null;
    ...
    for (var i = 0; i < 4; i++) {
        if (arrayMat[i] != null) arrayMat[i] = Math.abs(arrayMat[i] - Math.max(matIzq, matArr, matDer, matAba));
    ...
    for (var i = 0; i < 4; i++) {
        if (arrayMat[i] == Math.max(matIzq, matArr, matDer, matAba)) arrayMat[i] = arrayMat[i] + total;
    ...
    if (total != 0) {
        ...
        for (var i = 0; i < 4; i++) {
            if (arrayMat[i] != null) arrayMat[i] = 1;
            //console.log("bucle: "+arrayMat[i]);
        }
        total = arrayMat[0] + arrayMat[1] + arrayMat[2] + arrayMat[3];
        goLeft = arrayMat[0] / total;
    ...
    switch (true) {
        case probabilidad <= goLeft:
            //menos que goLeft
            if (goLeft != 0) return "LEFT";
            break;

        case probabilidad <= goLeft + goUp:
            //entre goLeft y goUp
            if (goUp != 0) return "UP";
            break;

        case probabilidad <= goLeft + goUp + goRight:
            //entre goUp y goRight
            if (goRight != 0) return "RIGHT";
            break;

        case probabilidad <= goLeft + goUp + goRight + goDown:
            //entre goRight y goDown
            if (goDown != 0) return "DOWN";
            break;
    }
}
