...
class CleanerProblem extends Problem {
    ...
    perceptionForAgent(data, agentID) {
        ...
        let result = [];
        //LEFT
        result.push(x > 0 ? map[y][x - 1] : "w");
        //UP
        result.push(y > 0 ? map[y - 1][x] : "w");
        //RIGTH
        result.push(x < map[0].length - 1 ? map[y][x + 1] : "w");
        //DOWN
        result.push(y < map.length - 1 ? map[y + 1][x] : "w");

        result = result.map(value => value != 0 ? value < 0 ? 0 : "w" : 0);
